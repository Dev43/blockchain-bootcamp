...
contract NHLPredictor is usingOraclize{
  ...
  event ApiKey(string _description);
  event BetPlaced(string indexed _betPlaced, string _team, string conference, uint _amountBet);
  ...
  function NHLPredictor(string _apiKey){
  ...
  // function addApiKey(string key){
  //   apiKey = key;
  // }



  function placeBet(string _forTeam, string _conference) payable {

    if(now > contractCreationTime + 10 minutes  || allBets[msg.sender].init != true){
      throw; // you can only bet once, and the betting is only for 7 days
    }
    uint amount = msg.value;
    contractBalance += amount;
    allBets[msg.sender] = Bet(true, _forTeam, _conference, amount);
  ...
  modifier isBettingDone(){
    if(now < contractCreationTime + 2 days){
