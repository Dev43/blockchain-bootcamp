...
contract NHLPredictor is usingOraclize{
  ...
  function getWinnerPerDivision(string _division) isBettingDone {
    ...
    if(sha3(_division) == sha3("pacific")){
      endOfURL = ".conferences[0].divisions[0].teams[0].name";
    } else if(sha3(_division) == sha3("central")){
      endOfURL = ".conferences[0].divisions[1].teams[0].name";
    } else if (sha3(_division) == sha3("atlantic")){
      endOfURL = ".conferences[1].divisions[0].teams[0].name";
    } else if (sha3(_division) == sha3("metropolitan")){
...
Instead of adding the 3 contracts that I did on the top (for the sake of clarity) we
can instead call a contract that is already on the blockchain

All you need is its address and either:

1) it's function definition  -- so only the name, its arguments and what it returns (Called abstract contract)
...
library strings {
  ...
  contract NHLPredictor{
   strings s = string(DEPLOYED_STRINGS_LIB_ADDRESS)
  }

2) Or its you can do it directly with

  .call -- but it is discouraged
